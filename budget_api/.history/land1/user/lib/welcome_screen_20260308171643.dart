...
import '../screens/submit_report_screen.dart';          // ← NEW
...
class _WelcomeScreenState extends State<WelcomeScreen> {
  ...
  // Handle moving rejected projects to the drawer section
  ...
  Widget _buildProjectLogicSection() {
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        return Column(
          children: [
            _createProjectButton(), // Always enabled and shown
            const SizedBox(height: 18),

            // Loop through all active projects instead of just the first one
  ...
  // Button is now universally available without a bool check
  Widget _createProjectButton() {
    return SizedBox(
      ...
      child: ElevatedButton.icon(
        ...
        label: const Text("Propose a new plan"),
  ...
  // ── UPDATED: 3 tabs — Home | Report | Profile ──────────────────────
  Widget _bottomNavBar() {
    return BottomNavigationBar(
      ...
      onTap: (index) {
        if (index == 1) {
          // Open Report screen — does NOT change _currentIndex so Home stays selected
          Navigator.push(
            context,
      ...
      items: const [
        ...
        BottomNavigationBarItem(icon: Icon(Icons.camera_alt_rounded), label: 'Report'),
