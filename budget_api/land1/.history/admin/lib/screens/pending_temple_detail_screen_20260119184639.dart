...
class PendingTempleDetailScreen extends StatelessWidget {
      ...
      children: [
        ...
        Expanded(
          child: OutlinedButton(
            // BUG FIXED HERE: Calling rejection handler instead of just local onDeleted
  ...
  // BUG FIX: Actually updating Firestore status to 'rejected'
  void _handleReject(BuildContext context) async {
    final confirm = await _showDialog(context, 'Reject Proposal', 'Are you sure you want to reject this proposal? It will be moved to the rejected list.', Colors.red);
    if (confirm == true) {
      final docId = (temple['id'] ?? temple['projectId'] ?? '').toString();
      if (docId.isEmpty) return;
      ...
      try {
        ...
        onUpdated(temple); // Updates the parent UI
        onDeleted(); // Triggers any cleanup if needed
