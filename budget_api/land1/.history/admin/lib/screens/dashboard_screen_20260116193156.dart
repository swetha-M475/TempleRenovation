    ...
    try {
      ...
      users = usersSnap.docs.map((doc) {
        ...
        return {
          ...
          'district': data['district'] ?? 'Not Specified', // Added District
          'taluk': data['taluk'] ?? 'Not Specified',       // Added Taluk
      ...
      for (final doc in projectsSnap.docs) {
        ...
        // Remove districts which do not have a proper name
        ...
        // Logic: Request is "New" if status is pending or not sanctioned
      ...
      // Convert map to list and SORT BY newRequests (Descending)
      districts = byDistrict.values.map((e) {
        e.remove('_placeSet');
        return e;
      }).toList();

      // Sorting Logic: New requests first
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(user['phone'], style: const TextStyle(color: Colors.black87)),
                  const Divider(height: 20),
                  // Replaced Address with District and Taluk
                  Row(
                    children: [
                      const Icon(Icons.map_outlined, size: 14, color: Color(0xFFB8962E)),
                      const SizedBox(width: 6),
                      Text(
                        "${user['taluk']}, ${user['district']}",
                        style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: Color(0xFF6D1B1B)),
                      ),
                    ],
                  ),
