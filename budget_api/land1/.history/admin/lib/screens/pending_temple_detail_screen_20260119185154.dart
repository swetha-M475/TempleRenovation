...
class PendingTempleDetailScreen extends StatelessWidget {
  ...
  final VoidCallback onDeleted; // This is the key to making it disappear
  ...
  void _handleSanction(BuildContext context) async {
    ...
    if (confirm == true) {
      ...
      // Update Firestore: it is no longer pending
      ...
      onUpdated(temple);
      onDeleted(); // Immediately removes it from the pending tab list
      Navigator.pop(context);
    }
  }
  ...
  void _handleReject(BuildContext context) async {
    ...
    if (confirm == true) {
      ...
      // Update Firestore: set status to rejected
      await FirebaseFirestore.instance
          .collection('projects')
          .doc(docId)
          .update({
      ...
      // Notify parent to remove this item from the local list
      onDeleted(); 
      // Update the local object just in case
      onUpdated(temple); 
