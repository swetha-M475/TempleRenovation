...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      // 1. Load Projects First to identify who has ongoing work
      final projectsSnap = await FirebaseFirestore.instance.collection('projects').get();
      
      // We will track users who have at least one project that is NOT pending
      final Set<String> usersWithOngoing = {};
      // We will track users who have submitted a proposal (any project linked to them)
      ...
      for (final doc in projectsSnap.docs) {
        ...
        if (userName.isNotEmpty) {
          ...
          // Logic: Project is ongoing if it is sanctioned or status is not pending
      ...
      // 2. Load Users and filter those who have submitted proposals
      ...
      users = usersSnap.docs
          ...
          .where((u) => u['hasProposal'] == true) // Only show users if a proposal exists
          .toList();
                      ...
                      Row(
                        children: [
                          ...
                          Text(
                            ...
                            style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: Color(0xFF4A1010)),
                          ),
                        ],
                      ),
                      TextButton.icon(
  ...
  void _showUserDetail(Map<String, dynamic> user) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Row(
          children: [
            Text(user['name'], style: const TextStyle(color: Color(0xFF6D1B1B))),
            if (user['isOngoing']) const Icon(Icons.flag, color: Colors.green),
          ],
        ),
        content: Column(
          mainAxisSize: MainAxisSize.min,
