...
class _FinanceTabSectionState extends State<FinanceTabSection> {
  ...
  Future<void> _launchUPI(String upiId, String amount, String title, {String? packageName}) async {
    ...
    if (Platform.isAndroid && packageName != null) {
      // Use the Intent scheme for specific Android Apps
      urlString = "intent://pay?$query#Intent;scheme=upi;package=$packageName;end";
    } else {
      // Standard scheme for iOS or generic chooser
    ...
    try {
      // Check if the app can be launched before trying
      if (await canLaunchUrl(uri)) {
        await launchUrl(uri, mode: LaunchMode.externalApplication);
      } else {
        _showErrorSnackBar(packageName != null 
            ? "The selected payment app is not installed." 
        ...
        return SafeArea(
          child: Column(
            ...
            children: [
              ...
              const Divider(),
              _buildAppTile("Google Pay", Icons.account_balance, Colors.blue, upiId, amount, title, docId, gpayPackage),
              _buildAppTile("PhonePe", Icons.account_balance_wallet, Colors.purple, upiId, amount, title, docId, phonepePackage),
              _buildAppTile("Paytm", Icons.payments, Colors.lightBlue, upiId, amount, title, docId, paytmPackage),
              _buildAppTile("Other UPI App", Icons.qr_code, Colors.grey, upiId, amount, title, docId, null),
              const Divider(),
              ListTile(
  ...
  Widget _buildAppTile(String name, IconData icon, Color color, String upiId,
      String amount, String title, String docId, String? packageName) {
    return ListTile(
      leading: CircleAvatar(backgroundColor: color.withOpacity(0.1), child: Icon(icon, color: color)),
      title: Text(name),
      onTap: () async {
        Navigator.pop(context);
        await _launchUPI(upiId, amount, title, packageName: packageName);
        _askIfPaid(docId);
      },
    );
  }
