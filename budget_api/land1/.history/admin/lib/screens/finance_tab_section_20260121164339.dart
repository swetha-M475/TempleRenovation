  ...
  void _askIfPaid(String docId, {bool isCash = false}) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: Text(isCash ? "Confirm Cash Payment" : "Enter Transaction Details"),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Text(isCash 
              ? "Mark this transaction as paid via Cash?" 
              : "Please enter the Transaction ID / Reference No. from your payment app."),
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () {
              if (!isCash && _transactionController.text.trim().isEmpty) {
                _showErrorSnackBar("Please enter the Transaction ID");
                return;
              }
              ...
              FirebaseFirestore.instance.collection('transactions').doc(docId).update({
                ...
                'transactionId': isCash ? 'CASH_PAYMENT' : _transactionController.text.trim(),
              });
              Navigator.pop(context);
            },
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (status == 'paid') ...[
                          ...
                          Text('Method: ${data['method'].toString().toUpperCase()}', 
                        ...
                        if (status == 'pending')
                          SizedBox(
                            ...
                            child: ElevatedButton.icon(
                              ...
                              label: const Text('PAY & MARK AS PAID', style: TextStyle(color: Colors.white)),
                            ),
                          ),
                      ],
                    ),
