...
class PendingTempleDetailScreen extends StatelessWidget {
                  ...
                  _buildSectionCard(
                    ...
                    child: Column(
                      children: [
                        ...
                        // FIX: Checks for 'userAadhar' first, falls back to 'aadhar'
                        _buildInfoRow('Aadhar', (temple['userAadhar'] ?? temple['aadhar'] ?? 'Not provided').toString()),
                      ],
                    ),
                  ),
  ...
  Widget _buildSiteImagesCard() {
    // FIX: Fallback logic for site images. 
    // Checks for 'imageUrls' first, then 'siteImages' from user app submission
    final dynamic rawImages = temple['imageUrls'] ?? temple['siteImages'];
    List<String> siteImages = [];
    
    if (rawImages is List) {
      siteImages = List<String>.from(rawImages);
    }
      ...
      child: siteImages.isEmpty
          ? const Center(
              child: Padding(
                ...
                child: Text('No site images provided'),
              ),
            )
          : SizedBox(
              height: 160,
  ...
  void _handleSanction(BuildContext context) async {
    ...
    if (confirm == true) {
      ...
      // Ensure we use the right document ID if 'id' is missing
      if (docId.isEmpty) {
