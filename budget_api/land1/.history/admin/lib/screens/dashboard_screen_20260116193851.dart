...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  // Helper to clean "null" strings from database
  ...
  Future<void> _loadData() async {
    ...
    try {
      // 1. Fetch Users and their specific details
      ...
      users = usersSnap.docs.map((doc) {
        ...
        return {
          'id': doc.id,
          'name': _formatValue(data['name']),
          'email': _formatValue(data['email']),
          'phone': _formatValue(data['phoneNumber'] ?? data['phone']),
          'district': _formatValue(data['district']), // Fetched from User document
          'taluk': _formatValue(data['taluk']),       // Fetched from User document
          'activeProjects': 0,
        };
      }).toList();

      // 2. Load Projects and Group by District
      ...
      for (final doc in projectsSnap.docs) {
        ...
        // Remove invalid districts from the Project List
        ...
        // Count as "New Request" if not yet sanctioned
      ...
      // SORTING: Districts with New requests first (Descending)
  ...
  Widget _buildUsersList() {
    final filtered = users.where((u) => u['name'].toString().toLowerCase().contains(searchQuery.toLowerCase()) || u['phone'].toString().contains(searchQuery)).toList();
    return ListView.builder(
      padding: const EdgeInsets.symmetric(horizontal: 16),
      itemCount: filtered.length,
      itemBuilder: (context, index) {
                ...
                children: [
                  Row(
                    ...
                    children: [
                      ...
                      Text(user['phone'], style: const TextStyle(color: Color(0xFF6D1B1B), fontWeight: FontWeight.w600, fontSize: 14)),
                    ],
                  ),
                  const Divider(height: 24, color: Color(0xFFF0E0C0)),
                  Row(
                    children: [
                      const Icon(Icons.location_on_outlined, size: 16, color: Color(0xFFB8962E)),
                      const SizedBox(width: 8),
                      Expanded(
                        child: Text(
                          "Taluk: ${user['taluk']}  |  District: ${user['district']}",
                          style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: Color(0xFF4A1010)),
                        ),
                      ),
                    ],
                  ),
  ...
  void _showUserDetail(Map<String, dynamic> user) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Text(user['name'], style: const TextStyle(color: Color(0xFF6D1B1B), fontWeight: FontWeight.bold)),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Mobile: ${user['phone']}', style: const TextStyle(fontSize: 16)),
            const SizedBox(height: 8),
            Text('District: ${user['district']}', style: const TextStyle(fontSize: 16)),
            const SizedBox(height: 8),
            Text('Taluk: ${user['taluk']}', style: const TextStyle(fontSize: 16)),
          ],
        ),
        actions: [
          TextButton(
          ...
          TextButton(
            ...
            child: const Text('Remove User', style: TextStyle(color: Colors.red, fontWeight: FontWeight.bold)),
          ),
        ],
      ),
    );
