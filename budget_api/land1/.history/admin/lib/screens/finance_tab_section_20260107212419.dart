...
class _FinanceTabSectionState extends State<FinanceTabSection> {
  ...
  // Updated helper to target specific Android packages
  Future<void> _launchUPI(String upiId, String amount, String title, {String? packageName}) async {
    ...
    if (Platform.isAndroid && packageName != null) {
      // Intent scheme forces Android to use the specific app package
      url = "intent://pay?$query#Intent;scheme=upi;package=$packageName;end";
    } else {
      // Fallback to generic scheme for iOS or 'Other' apps
      url = "upi://pay?$query";
    }

    try {
      final Uri uri = Uri.parse(url);
      // LaunchMode.externalApplication is required for Intent schemes to work
      await launchUrl(uri, mode: LaunchMode.externalApplication);
    } catch (e) {
      _showErrorSnackBar("Could not launch the selected payment app.");
    }
  }
        ...
        return SafeArea(
          child: Column(
            ...
            children: [
              ...
              const Divider(),
              // Added specific package names for Android targeting
  ...
  // Updated helper to accept package name 'pkg'
