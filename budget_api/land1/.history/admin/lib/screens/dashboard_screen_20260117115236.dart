...
import 'dart:async';
import 'package:rxdart/rxdart.dart'; // Recommended: add rxdart to your pubspec.yaml
...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  // Single subscription to handle combined streams
  ...
  /// Combined Listeners: Updates the UI immediately when either 
  /// the 'projects' or 'users' collection changes in Firestore.
  void _startRealTimeListeners() {
    // We use CombineLatestStream so that if EITHER collection changes, 
    // the UI processes the latest data from both.
    final projectsStream = FirebaseFirestore.instance.collection('projects').snapshots();
    final usersStream = FirebaseFirestore.instance.collection('users').snapshots();

    _combinedSubscription = Rx.combineLatest2(
      projectsStream,
      usersStream,
      (QuerySnapshot projects, QuerySnapshot users) => {
        'projects': projects,
        'users': users,
      },
    ).listen((data) {
      _processData(data['projects']!, data['users']!);
    }, onError: (error) {
  ...
  void _processData(QuerySnapshot projectsSnap, QuerySnapshot usersSnap) {
    try {
      ...
      for (final doc in projectsSnap.docs) {
        ...
        // Logic for "NEW" badge: showing proposals that aren't sanctioned yet
