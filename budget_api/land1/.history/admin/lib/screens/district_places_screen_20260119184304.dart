...
class _DistrictPlacesScreenState extends State<DistrictPlacesScreen> {
  ...
  Future<void> _loadPlaces() async {
    ...
    try {
      // UPDATED QUERY: Added filter to ignore rejected proposals
      ...
      for (final doc in snapshot.docs) {
        ...
        // Count as new request only if it's not sanctioned AND status is pending
        if (!isSanctioned && data['status'] == 'pending') {
  ...
  Widget _buildTalukCard(Map<String, dynamic> place) {
    return Card(
      ...
      child: ListTile(
        onTap: () async {
          // Navigating and then refreshing on return to catch deletions
