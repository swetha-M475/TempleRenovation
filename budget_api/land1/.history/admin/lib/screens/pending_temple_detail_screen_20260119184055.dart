...
class PendingTempleDetailScreen extends StatelessWidget {
    ...
    return _buildSectionCard(
      ...
      child: Column(
        ...
        children: [
          _buildInfoRow(
            'Local POC',
            (temple['contactName'] ?? temple['userName'] ?? 'N/A').toString(),
          ),
      ...
      children: [
        Expanded(
          child: ElevatedButton(
            ...
            child: const Text('SANCTION', style: TextStyle(fontWeight: FontWeight.bold)),
          ),
        ),
        const SizedBox(width: 12),
        Expanded(
          child: OutlinedButton(
            onPressed: () => _handleReject(context), // Updated to handle rejection logic
  ...
  void _handleSanction(BuildContext context) async {
    ...
    if (confirm == true) {
      ...
      onUpdated(null); // Notify parent to remove from "Pending" list
      if (context.mounted) Navigator.pop(context);
    }
  }

  // FIXED: Logic to update Firestore and remove from Admin View
  void _handleReject(BuildContext context) async {
    final confirm = await _showDialog(
      context,
      'Reject Proposal',
      'Are you sure you want to reject this proposal? It will be removed from your pending list.',
      Colors.red,
    );
    if (confirm == true) {
      final docId = (temple['id'] ?? temple['projectId'] ?? '').toString();
      ...
      // Update Firestore so it no longer meets the "pending" query criteria
      await FirebaseFirestore.instance
          .collection('projects')
          .doc(docId)
          .update({'status': 'rejected'});

      // Trigger the delete callback to refresh parent state immediately
      onDeleted(); 
