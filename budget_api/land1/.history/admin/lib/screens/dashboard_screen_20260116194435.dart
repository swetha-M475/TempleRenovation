...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  // Remove user logic (removes user document and associated projects)
  Future<void> _handleRemoveUser(Map<String, dynamic> user) async {
    final confirm = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Confirm Deletion'),
        content: Text('Are you sure you want to remove ${user['name']}? This will also delete all projects submitted by this user.'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Cancel')),
          TextButton(onPressed: () => Navigator.pop(context, true), child: const Text('Remove Everything', style: TextStyle(color: Colors.red))),
        ],
      ),
    );
    ...
    if (confirm == true) {
      ...
      try {
        ...
        // 1. Find and mark projects for deletion
        // Note: Filtering by 'userName' or 'userId'. Adjust the key to match your project schema.
        ...
        // Refresh project/district counts
        await _loadData();
        
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('User and their projects removed successfully')));
      } catch (e) {
        debugPrint('Error removing user data: $e');
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Error during removal process')));
        setState(() => isLoading = false);
      }
    }
  }
