...
class PendingTempleDetailScreen extends StatelessWidget {
  ...
  Widget _buildFeaturesCard() {
    ...
    if (raw is List) {
      features = raw.map((e) => (e as Map).map((k, v) => MapEntry(k.toString(), v))).toList();
    }
              ...
              children: features.map((f) {
                ...
                return Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(color: isNew ? const Color(0xFF2D6A4F) : Colors.grey.shade300),
                  ),
                  child: Row(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(label, style: GoogleFonts.poppins(fontWeight: FontWeight.w600, fontSize: 14, color: maroonDeep)),
                            Text(statusText, style: GoogleFonts.poppins(fontSize: 11, color: isNew ? const Color(0xFF2D6A4F) : Colors.grey)),
                            if (isNew && dimension.isNotEmpty)
                              Text('Size: ${dimension == 'custom' && customSize.isNotEmpty ? customSize : dimension}', style: GoogleFonts.poppins(fontSize: 12, color: textDark)),
                            if (isNew && amount.isNotEmpty)
                              Text('Required amount: ₹$amount', style: GoogleFonts.poppins(fontSize: 12, color: textDark)),
                          ],
                        ),
                      ),
                    ],
              ...
              child: ListView.separated(
                ...
                itemBuilder: (context, index) {
                  ...
                  return GestureDetector(
                    onTap: () {
                      Navigator.of(context).push(MaterialPageRoute(builder: (_) => FullscreenImageViewer(source: source)));
                    },
                    child: ClipRRect(
                      ...
                      child: Container(
                        ...
                        child: isUrl ? _buildNetworkImage(source) : _buildBase64Image(source, isDataUri: isDataUri),
                      ),
                    ),
                  );
                },
  ...
  Widget _buildBase64Image(String base64String, {bool isDataUri = false}) {
    try {
      ...
      if (isDataUri || base64String.contains(',')) cleanHash = base64String.split(',').last;
      cleanHash = cleanHash.replaceAll('\n', '').replaceAll('\r', '').trim();
      final Uint8List bytes = base64Decode(cleanHash);
      return Image.memory(bytes, fit: BoxFit.cover, width: 280, height: 200);
    } catch (e) {
  ...
  void _handleSanction(BuildContext context) async {
    ...
    if (confirm == true) {
      ...
      try {
        // FIXED: Syncing status with WelcomeScreen expectations
        // We use 'sanctioned' as the status to match the logic in the WelcomeScreen
        ...
        // Update local map before callback
        temple['isSanctioned'] = true;
        temple['status'] = 'sanctioned';
        
        onUpdated(temple);
        onDeleted(); // Remove from the "Pending" list view
        Navigator.pop(context);
      } catch (e) {
        debugPrint("Error sanctioning project: $e");
      }
    }
  }

  void _handleReject(BuildContext context) async {
    final confirm = await _showDialog(context, 'Reject Project', 'Are you sure you want to reject this project?', Colors.red);
    if (confirm == true) {
      final docId = (temple['id'] ?? temple['projectId'] ?? '').toString();
      if (docId.isEmpty) return;
      ...
      try {
        ...
        temple['isSanctioned'] = false;

        onDeleted(); // Remove from Pending list
        Navigator.pop(context);
...
class FullscreenImageViewer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Widget child;
    if (isUrl) {
      child = InteractiveViewer(child: Image.network(source, fit: BoxFit.contain));
    } else {
      String clean = source;
      if (isDataUri || source.contains(',')) clean = source.split(',').last;
