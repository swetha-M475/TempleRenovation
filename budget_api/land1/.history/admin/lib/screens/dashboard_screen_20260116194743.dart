...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  bool _isValidDistrict(String name) {
    ...
    // Rejects names shorter than 3 chars (like 'a') or common dummy names
    if (cleanName.length < 3) return false;
    if (cleanName == 'null' || cleanName == 'test' || cleanName == 'undefined') return false;
    return true;
  }
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      // 2. Load Projects and Perform Cleanup
      ...
      for (final doc in projectsSnap.docs) {
        ...
        // CLEANUP LOGIC: Delete projects with garbage district names (like 'a') from Firebase
      ...
      // Execute deletion of unwanted districts from Firebase
      if (needsCleanup) {
        await cleanupBatch.commit();
        debugPrint('Cleaned up unwanted district entries from Firebase.');
      }
  ...
  // Remove user logic (Cascade deletes projects)
  Future<void> _handleRemoveUser(Map<String, dynamic> user) async {
    ...
    if (confirm == true) {
      try {
        ...
        // Delete user project references
