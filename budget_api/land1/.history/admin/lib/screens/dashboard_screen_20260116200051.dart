...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  // Filter logic: ONLY allow "Chennai". Remove everything else.
  bool _isAllowedDistrict(String name) {
    final cleanName = name.trim().toLowerCase();
    return cleanName == 'chennai';
  }
    ...
    try {
      ...
      users = usersSnap.docs.map((doc) {
        ...
        return {
          ...
          'district': data['district'] ?? data['districtName'] ?? 'Not Set',
          'taluk': data['taluk'] ?? data['talukName'] ?? 'Not Set',
        };
      }).toList();
      ...
      // 2. Load Projects and Filter for Chennai only
      ...
      // Batch for cleaning up the "error" districts from database if needed
      ...
      for (final doc in projectsSnap.docs) {
        ...
        // If it is NOT Chennai, we skip it (and optionally delete from DB)
        if (!_isAllowedDistrict(districtName)) {
          // Uncomment the lines below if you want to delete these error districts from Firebase permanently:
      ...
      // Sort districts by new requests
  ...
  Widget _buildDistrictsList() {
    ...
    if (filtered.isEmpty) {
      return const Center(child: Text("No data found for Chennai"));
    }
                ...
                Row(
                  children: [
                    ...
                    Text(
                      ...
                      style: const TextStyle(fontSize: 13, color: Color(0xFF4A1010)),
                    ),
                  ],
                ),
                const Divider(height: 24, color: Color(0xFFFFF4D6)),
                Align(
  ...
  Future<void> _handleRemoveUser(Map<String, dynamic> user) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Are you sure you want to remove ${user['name']} and all their projects?'),
        actions: [
    ...
    if (confirm == true) {
      try {
        ...
        _loadData(); 
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('User and projects removed')));
      } catch (e) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Error removing user')));
      }
    }
  }
