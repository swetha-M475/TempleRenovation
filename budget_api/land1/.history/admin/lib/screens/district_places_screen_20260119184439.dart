...
class _DistrictPlacesScreenState extends State<DistrictPlacesScreen> {
  ...
  Future<void> _loadPlaces() async {
    ...
    try {
      ...
      for (final doc in snapshot.docs) {
        ...
        // BUG FIX: Skip this project if it is rejected
        ...
        // A project is a "New Request" only if it's not sanctioned and is pending
      ...
      if (mounted) {
        setState(() {
          // If all projects in a taluk were rejected, the taluk won't exist in talukMap
  ...
  Widget _buildTalukCard(Map<String, dynamic> place) {
    return Card(
      ...
      child: ListTile(
        onTap: () async {
          // Wait for the user to come back from the temple list
          ...
          // Refresh the list when coming back to remove taluks if all temples were rejected
          _loadPlaces();
        },
        title: Text(
          place['name'],
