...
class _TempleDetailScreenState extends State<TempleDetailScreen> {
  ...
  bool isLoading = true;
  int selectedTab = 0; // 0=activities, 1=bills, 2=suggestions
  ...
  Future<void> _loadTemple() async {
    ...
    temple!['lingamDimensions'] =
        (temple!['lingamDimensions'] ?? '-') as String;
    temple!['avudaiType'] = (temple!['avudaiType'] ?? '') as String;
    temple!['avudaiDimensions'] =
        (temple!['avudaiDimensions'] ?? '-') as String;
    temple!['nandhiType'] = (temple!['nandhiType'] ?? '') as String;
    temple!['nandhiDimensions'] =
        (temple!['nandhiDimensions'] ?? '-') as String;
    temple!['estimatedAmount'] =
        (temple!['estimatedAmount'] ?? 0.0) as num;
    temple!['completedDate'] = temple!['completedDate'];
    temple!['siteImages'] =
        List<String>.from(temple!['siteImages'] ?? <String>[]);
    ...
    activities = [
      ...
      },
      {
        ...
        'status': 'completed',
        'images': [
          'https://picsum.photos/seed/activity3/400/250',
        ],
      },
    ];
  ...
  Future<void> _handleReject() async {
    ...
    if (confirm == true) {
      // TODO: Backend delete/reject
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Project rejected and deleted!')),
      );
      // Return null so parent removes this temple
  ...
  Future<void> _handleMarkCompleted() async {
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Are you sure you want to mark this project as completed?'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context, false),
            child: const Text('Cancel'),
  ...
  void _showActivityDetail(Map<String, dynamic> activity) {
    final List<String> images =
        List<String>.from(activity['images'] ?? <String>[]);
            ...
            Expanded(
              child: ListView(
                ...
                children: [
                  ...
                  if (isOngoing) ..._buildOngoingView(),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildInfoRow(
                'Name:', (temple!['userName'] ?? '') as String),
            _buildInfoRow(
                'Email:', (temple!['userEmail'] ?? '') as String),
            _buildInfoRow(
                'Phone:', (temple!['userPhone'] ?? '') as String),
            _buildInfoRow(
                'Aadhar:', (temple!['userAadhar'] ?? '') as String),
          ],
        ),
      ),
    );
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const Divider(height: 24),
              _buildInfoRow(
                  'Village:', (temple!['village'] ?? '') as String),
              _buildInfoRow(
                  'Town:', (temple!['town'] ?? '') as String),
              _buildInfoRow(
                  'Taluk:', (temple!['taluk'] ?? '') as String),
              _buildInfoRow(
                  'District:', (temple!['district'] ?? '') as String),
              _buildInfoRow(
                  'Visit Date:', (temple!['visitDate'] ?? '') as String),
            ],
          ),
        ),
      ),
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              _buildComponentCard(
                'Lingam',
                (temple!['lingamType'] ?? '') as String,
                (temple!['lingamDimensions'] ?? '-') as String,
              ),
              const SizedBox(height: 12),
              _buildComponentCard(
                'Avudai',
                (temple!['avudaiType'] ?? '') as String,
                (temple!['avudaiDimensions'] ?? '-') as String,
              ),
              const SizedBox(height: 12),
              _buildComponentCard(
                'Nandhi',
                (temple!['nandhiType'] ?? '') as String,
                (temple!['nandhiDimensions'] ?? '-') as String,
              ),
            ],
          ),
  ...
  List<Widget> _buildOngoingView() {
    final totalSpent = transactions.fold<num>(
      0,
      (prev, t) => prev + (t['amount'] ?? 0) as num,
    );
      ...
      return [
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              const Text(
                'All work completed for this temple?',
                style: TextStyle(
                    color: Colors.white,
                    fontSize: 14),
              ),
              ...
              children: [
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    hintText:
                        'Type your suggestion for the user here...',
                    border: OutlineInputBorder(
                      borderRadius:
                          BorderRadius.circular(8),
                    ),
    ...
    return Column(
      children: transactions.map((t) {
        ...
        return ListTile(
          ...
          title: Text(
            '₹${amount.toStringAsFixed(0)} - $description',
            style: const TextStyle(
                fontWeight: FontWeight.w600),
          ),
          subtitle: Text(
            '${paymentMode.toUpperCase()} • $date',
            style: const TextStyle(
                fontSize: 12,
                color: Colors.grey),
          ),
