...
class _SignupScreenState extends State<SignupScreen> {
  ...
  // Helper to check if a specific field is valid for the trailing icon
  bool _isValid(String type, String value) {
    if (value.isEmpty) return false;
    switch (type) {
      case "name": return value.length >= 3 && RegExp(r'^[a-z A-Z]+$').hasMatch(value);
      case "username": return value.length >= 4 && RegExp(r'^[a-zA-Z0-9_]+$').hasMatch(value);
      case "phone": return value.length == 10;
      case "aadhar": return value.length == 12;
                  ...
                  child: Column(
                    children: [
                      ...
                      Container(
                        ...
                        child: Form(
                          key: _formKey,
                          autovalidateMode: AutovalidateMode.onUserInteraction, // Shows error below field
                          child: Column(
                            children: [
                              _input(_name, "Full Name", Icons.person_outline, validationType: "name"),
                              const SizedBox(height: 16),
                              _input(_username, "Username", Icons.alternate_email, validationType: "username"),
                              const SizedBox(height: 16),
                              _phoneSection(),
                              const SizedBox(height: 16),
            ...
            children: [
              Expanded(
                child: TextFormField(
                  ...
                  validator: (v) => (v == null || v.length != 10) ? "Enter 10 digit phone number" : null,
                ),
              ),
