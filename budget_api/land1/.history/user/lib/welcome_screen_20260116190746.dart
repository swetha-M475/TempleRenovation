    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        if (hasDocs) {
          ...
          // Use the Firestore document id as projectId – this must match project_tasks.projectId
          projectData = {
            'id': doc.id,
            'projectId': doc.id,
          ...
          status =
              (projectData['status'] ?? 'pending').toString().toLowerCase();
        }

        // User can propose again when there is no project, or the last was rejected or completed
        final bool canPropose =
            !hasDocs || status == 'rejected' || status == 'completed';
                ...
                Expanded(
                  child: Text(
                    status == 'rejected'
                        ...
                        : status == 'completed'
                            ? 'Project completed. Tap to view summary and works.'
