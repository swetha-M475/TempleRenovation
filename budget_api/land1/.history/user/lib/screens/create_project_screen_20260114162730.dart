...
class _CreateProjectScreenState extends State<CreateProjectScreen> {
  ...
  // Track if profile data was successfully fetched to keep fields locked
      ...
      if (user != null) {
        ...
        if (doc.exists && doc.data() != null) {
          ...
          setState(() {
            ...
            // Fetch Phone: Checks multiple common keys
            String? fetchedPhone = data['phone']?.toString() ?? 
                                   data['phoneNumber']?.toString() ?? 
                                   data['mobile']?.toString();
            
            // 2. Fallback to Firebase Auth phone number if Firestore is empty
            _contactPhoneController.text = fetchedPhone ?? user.phoneNumber ?? '';
            
            _isProfileFetched = true;
          });
        } else if (user.phoneNumber != null) {
          // 3. Last resort fallback if document doesn't exist
          setState(() {
  ...
  bool _validateCurrentPage() {
    ...
    if (_currentPage == 2) {
      if (_contactNameController.text.trim().isEmpty) return _showWarning('Contractor name missing from profile');
      if (_contactPhoneController.text.trim().isEmpty) return _showWarning('Phone number missing from profile');
    ...
    return Padding(
      ...
      child: TextFormField(
        ...
        decoration: InputDecoration(
          ...
          filled: true,
          // Background turns slightly grey if readOnly to signal it's "verified/locked"
