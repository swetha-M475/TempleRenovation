...
class _ProjectOverviewScreenState extends State<ProjectOverviewScreen> {
    ...
    showModalBottomSheet(
      ...
      builder: (context) => Padding(
        ...
        child: Column(
          ...
          children: [
            const Text("AI Receipt Summary", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
            const SizedBox(height: 16),
            Container(
              padding: const EdgeInsets.all(12),
              decoration: BoxDecoration(color: Colors.grey[100], borderRadius: BorderRadius.circular(8)),
              child: Text(data, style: const TextStyle(fontFamily: 'monospace')),
            ),
          ],
        ),
      ),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("Project Overview", style: TextStyle(color: Colors.black)),
        backgroundColor: Colors.white,
        elevation: 0,
        actions: [
          IconButton(
  ...
  Widget _buildProgressCard() {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text("Overall Completion", style: TextStyle(color: Colors.white70, fontSize: 16)),
          const SizedBox(height: 8),
          const Text("74%", style: TextStyle(color: Colors.white, fontSize: 36, fontWeight: FontWeight.bold)),
          const SizedBox(height: 16),
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        return ListTile(
          ...
          trailing: Text(expenses[index]["amount"]!, style: const TextStyle(fontWeight: FontWeight.bold)),
        );
      },
    );
  }
