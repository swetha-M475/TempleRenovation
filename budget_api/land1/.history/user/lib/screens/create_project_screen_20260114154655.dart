...
class _CreateProjectScreenState extends State<CreateProjectScreen> {
  ...
  bool _validateCurrentPage() {
    if (_currentPage == 0) {
      return _placeController.text.trim().isNotEmpty &&
          ...
          _mapLocationController.text.trim().isNotEmpty &&
          _selectedDate != null;
    } else if (_currentPage == 1) {
      if (_selectedFeature == null) return false;
      if (_type == null) return false;
      if (_type == 'new') {
        if (_dimension == null) return false;
        if (_dimension == 'custom' && (_customDimensionController.text.isEmpty || _featureAmountController.text.isEmpty)) return false;
      }
      return true;
  ...
  Future<void> _detectAndFillLocation() async {
    ...
    try {
      ...
      String latDMS = _toDMS(position.latitude, true);
      String lngDMS = _toDMS(position.longitude, false);
  ...
  Future<void> _createProject() async {
    if (!_validateCurrentPage()) {
      _showWarning('Please complete all details including images');
      return;
    }
    ...
    try {
      ...
      await FirebaseFirestore.instance.collection('projects').doc(projectId).set({
        ...
        'featureType': _type,
        'featureDimension': _dimension == 'custom' ? _customDimensionController.text : _dimension,
      ...
      children: [
        ...
        if (_type == 'new') ...[
          ...
          ..._predefinedDimensions.map((dim) => Padding(
            ...
            child: _buildDimensionOption(dim['name'], 'Estimate: ₹${dim['amount']}', dim['name']),
          )),
          ...
          if (_dimension == 'custom') ...[
            ...
            const SizedBox(height: 12),
            _textField(_featureAmountController, 'Req. Amount (₹)', Icons.currency_rupee, keyboard: TextInputType.number),
          ],
        ],
      ],
    );
            ...
            child: ElevatedButton(
              onPressed: _isLoading ? null : () {
                if (_validateCurrentPage()) {
                  ...
                  _showWarning('Please fill all required fields correctly');
                }
              },
