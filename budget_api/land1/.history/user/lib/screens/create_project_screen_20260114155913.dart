  ...
  bool _validateCurrentPage() {
    ...
    if (_currentPage == 1) {
      ...
      if (_type == 'new') {
        ...
        if (_dimension == 'custom') {
          ...
          if (amt == null || amt <= 0) return _showWarning('Please enter a valid amount');
        }
      }
      return true;
    } 
    ...
    if (_currentPage == 2) {
      ...
      if (totalAmt == null || totalAmt <= 0) {
        return _showWarning('Enter total estimated cost');
      }

      // Check for at least 5 images
      if (_selectedImages.length < 5) return _showWarning('At least 5 photos are required');
      return true;
    }
    return false;
  }
    ...
    return _buildFormContainer(
      child: Column(
        children: [
          ...
          const SizedBox(height: 20),
          Text('Upload Photos (Min 5 Required)', style: GoogleFonts.poppins(fontSize: 14, fontWeight: FontWeight.bold)),
          const SizedBox(height: 8),
          ImagePickerWidget(
            maxImages: 10,
