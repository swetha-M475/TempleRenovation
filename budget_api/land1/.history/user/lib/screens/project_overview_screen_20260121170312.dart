...
class _ProjectOverviewScreenState extends State<ProjectOverviewScreen>
  ...
  Widget _transactionsTab() {
    // We stream the specific project document to get the "total amount" from the proposal
    return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, projectSnapshot) {
        ...
        // Fetching the total budget from the database (project document)
        final projectData = projectSnapshot.data!.data();
        final double totalBudget = double.tryParse(projectData?['budget']?.toString() ?? '0') ?? 0.0;
        ...
        return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
          ...
          builder: (context, transactionSnapshot) {
            ...
            final allDocs = transactionSnapshot.data!.docs;
            ...
            // Calculate Remaining Budget based on the LIVE total budget from DB
            double totalPaid = 0;
            for (var doc in paidDocs) {
              totalPaid += (doc['amount'] ?? 0).toDouble();
            }
            double remainingBudget = totalBudget - totalPaid;
