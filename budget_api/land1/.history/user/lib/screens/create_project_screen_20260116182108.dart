...
class _CreateProjectScreenState extends State<CreateProjectScreen> {
  ...
  // Logic for District -> Taluk mapping
  final Map<String, List<String>> _districtTalukMap = {
    'Ariyalur': ['Ariyalur', 'Sendurai', 'Udayarpalayam', 'Andimadam'],
    'Chengalpattu': ['Chengalpattu', 'Cheyyur', 'Maduranthakam', 'Pallavaram', 'Tambaram', 'Thiruporur'],
    'Chennai': ['Ayanavaram', 'Egmore', 'Guindy', 'Mylapore', 'Perambur', 'Tondiarpet', 'Velachery'],
    'Coimbatore': ['Pollachi', 'Mettupalayam', 'Sulur', 'Valparai', 'Perur', 'Madukkarai'],
    'Cuddalore': ['Cuddalore', 'Chidambaram', 'Panruti', 'Vridhachalam', 'Tittakudi', 'Kattumannarkoil'],
    'Dharmapuri': ['Dharmapuri', 'Harur', 'Pappireddipatti', 'Pennagaram', 'Palacode'],
    'Dindigul': ['Dindigul', 'Palani', 'Kodaikanal', 'Oddanchatram', 'Natham', 'Nilakottai'],
    'Erode': ['Erode', 'Bhavani', 'Gobichettipalayam', 'Sathyamangalam', 'Perundurai', 'Anthiyur'],
    'Madurai': ['Madurai North', 'Madurai South', 'Melur', 'Thirumangalam', 'Usilampatti', 'Vadipatti'],
    'Thanjavur': ['Thanjavur', 'Kumbakonam', 'Papanasam', 'Pattukkottai', 'Orathanadu', 'Thiruvaiyaru'],
    // Note: You can expand this list for all 38 districts
  };
  ...
  bool _validateCurrentPage() {
    ...
    if (_currentPage == 1) {
      ...
      if (newOnes.isEmpty) return _showWarning('At least one feature must be marked as New.');
      for (final f in newOnes) {
        if (f.dimension == null || f.dimension!.isEmpty) return _showWarning('Select dimension for ${f.label}.');
        if (f.dimension == 'custom' && (f.customSize == null || f.customSize!.trim().isEmpty)) {
          return _showWarning('Enter custom size for ${f.label}.');
        }
        if (f.amount == null || f.amount!.trim().isEmpty) return _showWarning('Enter required amount for ${f.label}.');
      }
      return true;
    }
    ...
    try {
      ...
      for (final imgPath in _selectedImages) {
        try {
          ...
          if (url != null && url.isNotEmpty) uploadedImageUrls.add(url);
        } catch (e) {
      ...
      if (uploadedImageUrls.length < 5) {
        _showWarning('At least 5 photos must upload successfully.');
        setState(() => _isLoading = false);
        return;
      }
    ...
    return _buildFormContainer(
      child: Column(
        children: [
          ...
          _buildDistrictField(),
          _buildTalukField(),
          _buildNearbyTownField(),
          _plainTextField(_stateController, 'State', readOnly: true),
          Row(
            children: [
              Expanded(
                child: _plainTextField(_mapLocationController, 'Map Location', readOnly: true),
              ),
  ...
  Widget _buildDistrictField() {
    return _customAutocomplete(
      ...
      options: _tnDistricts,
      onSelected: (val) {
        setState(() {
          _districtController.text = val;
          _talukController.clear(); // Reset child fields
          _nearbyTownController.clear();
        });
      },
    );
  }
  ...
  Widget _buildTalukField() {
    ...
    List<String> taluks = _districtTalukMap[_districtController.text] ?? [];
    return _customAutocomplete(
      label: 'Taluk',
      controller: _talukController,
      options: taluks,
      onSelected: (val) => _talukController.text = val,
      enabled: _districtController.text.isNotEmpty,
      hint: _districtController.text.isEmpty ? 'Select District first' : null,
    );
  }

  Widget _buildNearbyTownField() {
    // Usually Nearby Towns are also Taluks or Major centers in that district
    List<String> towns = _districtTalukMap[_districtController.text] ?? [];
    return _customAutocomplete(
      label: 'Nearby Town',
      controller: _nearbyTownController,
      options: towns,
      onSelected: (val) => _nearbyTownController.text = val,
      enabled: _districtController.text.isNotEmpty,
      hint: _districtController.text.isEmpty ? 'Select District first' : null,
    );
  }
    ...
    return Padding(
      ...
      child: Autocomplete<String>(
        ...
        optionsBuilder: (TextEditingValue textEditingValue) {
          if (textEditingValue.text == '' || !enabled) {
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  color: entry.condition == 'old' ? Colors.grey.shade200 : Colors.green.shade100,
                  borderRadius: BorderRadius.circular(6),
                ),
                child: Text(
                  entry.condition == 'old' ? 'Old' : 'New',
                  style: TextStyle(fontSize: 10, color: entry.condition == 'old' ? Colors.grey.shade800 : Colors.green.shade800, fontWeight: FontWeight.w600),
                ),
              ),
            ],
          ),
          ...
          if (entry.condition == 'new') ...[
            ...
            ..._predefinedDimensions.map((dim) => _buildFeatureDimensionTile(
              entry: entry,
              value: dim['name'],
              title: dim['name'],
            ...
            _buildFeatureDimensionTile(
              ...
              onSelected: () => setState(() => entry.dimension = 'custom'),
            ),
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: isSelected ? const Color(0xFF5D4037) : const Color(0xFFF5E6CA)),
        ),
        child: Row(
          children: [
            Icon(isSelected ? Icons.radio_button_checked : Icons.radio_button_off, size: 18, color: const Color(0xFF5D4037)),
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(8),
          border: Border.all(color: selected ? const Color(0xFF5D4037) : const Color(0xFFF5E6CA)),
        ),
        child: Row(
          children: [
            Icon(selected ? Icons.check_circle : Icons.circle_outlined, size: 20, color: const Color(0xFF5D4037)),
