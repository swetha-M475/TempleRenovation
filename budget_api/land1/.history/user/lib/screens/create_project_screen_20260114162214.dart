...
class _CreateProjectScreenState extends State<CreateProjectScreen> {
  ...
  bool _isFetchingProfile = true; // Track profile loading
    ...
    try {
      ...
      if (user != null) {
        ...
        if (doc.exists && doc.data() != null) {
          final data = doc.data()!;
          setState(() {
            // Updated to check multiple common key names for reliability
            _contactNameController.text = (data['name'] ?? data['fullName'] ?? '').toString();
            
            // Checking common phone keys: 'phone', 'phoneNumber', or the auth number itself
  ...
  bool _validateCurrentPage() {
    ...
    if (_currentPage == 2) {
      // Logic: If profile data is missing, we shouldn't let them submit
      if (_contactNameController.text.trim().isEmpty || _contactPhoneController.text.trim().isEmpty) {
        return _showWarning('Verified profile data missing. Please update profile.');
      }
    ...
    return Padding(
      ...
      child: TextFormField(
        ...
        decoration: InputDecoration(
          ...
          filled: true,
          fillColor: readOnly ? const Color(0xFFF0F0F0) : Colors.white, // Visual indicator for read-only
  ...
  Widget _buildContactPage() {
    return _buildFormContainer(child: Column(children: [
      ...
      const SizedBox(height: 16),
      // --- READONLY FIELDS ---
      _plainTextField(_contactNameController, 'Contractor Name', readOnly: true),
      ...
      child: Row(children: [
        ...
        Expanded(
          child: ElevatedButton(
            onPressed: _isLoading || _isFetchingProfile ? null : () {
