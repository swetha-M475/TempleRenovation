      ...
      children: [
        ...
        Padding(
          ...
          child: Text(
            "Transaction History",
            style: GoogleFonts.poppins(
                fontWeight: FontWeight.bold, fontSize: 16, color: primaryMaroon),
          ),
        ),
        Expanded(
          child: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
            ...
            builder: (context, snapshot) {
              ...
              return ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  final title = data['title'] ?? 'Requested Amount';
                  final date = _formatTimestamp(data['date'] as Timestamp?);
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      leading: CircleAvatar(
                        ...
                        child: Icon(
                          status == 'completed' ? Icons.check : Icons.history,
                          color: statusColor,
                        ),
                      ),
                      title: Text(title,
...
class _TabBarDelegate extends SliverPersistentHeaderDelegate {
  ...
  Widget build(
    ...
    return Container(
      color: ProjectOverviewScreen.backgroundCream,
      child: tabBar,
    );
  }
...
class _OngoingTaskCardState extends State<OngoingTaskCard> {
  ...
  Future<void> _submitForApproval() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Sent for admin approval!')),
        );
      }
    } catch (e) {
      if (mounted) {
          ...
          children: [
            ListTile(
              ...
              trailing: _isUploading
                  ...
                  : ElevatedButton(
                      onPressed: isPending ? null : _submitForApproval,
                      style: ElevatedButton.styleFrom(
                        backgroundColor: isPending ? Colors.orange : Colors.blue,
                        foregroundColor: Colors.white,
                      ),
                      child: Text(isPending ? 'Pending' : 'Finish'),
                    ),
            ),
            if (isPending)
              const Text(
                "Waiting for admin to verify and complete this work.",
                style: TextStyle(
                    fontSize: 11,
                    color: Colors.orange,
