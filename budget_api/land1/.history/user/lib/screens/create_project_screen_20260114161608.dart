...
class _CreateProjectScreenState extends State<CreateProjectScreen> {
  ...
  Future<void> _fetchContractorData() async {
    try {
      ...
      if (user != null) {
        // Fetching from 'users' collection using UID
        final doc = await FirebaseFirestore.instance.collection('users').doc(user.uid).get();
        if (doc.exists && doc.data() != null) {
          final data = doc.data()!;
          setState(() {
            // Mapping values from Firebase - adjust keys 'name'/'phone' if your DB uses different names
  ...
  bool _validateCurrentPage() {
    ...
    if (_currentPage == 2) {
      if (_contactNameController.text.trim().isEmpty || _contactPhoneController.text.trim().isEmpty) {
        return _showWarning('Contractor data missing from profile');
      }
    ...
    return Padding(
      ...
      child: TextFormField(
        ...
        decoration: InputDecoration(
          ...
          filled: true,
          fillColor: readOnly ? Colors.grey[50] : Colors.white, // Visual cue for locked fields
  ...
  Widget _buildContactPage() {
    return _buildFormContainer(child: Column(children: [
      ...
      const SizedBox(height: 16),
      // REQUIREMENT: Fixed Name & Phone (Locked)
      _plainTextField(_contactNameController, 'Contact Name', readOnly: true),
      _plainTextField(_contactPhoneController, 'Phone Number', keyboard: TextInputType.phone, readOnly: true),
