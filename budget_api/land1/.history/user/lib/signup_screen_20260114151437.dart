...
class _SignupScreenState extends State<SignupScreen> {
  ...
  void initState() {
    ...
    // Check phone when focus is lost OR when 10 digits are reached (handled in onChanged)
  ...
  Future<void> _checkDuplicate(String field, String value) async {
    ...
    if (field == "phoneNumber") setState(() => _checkingPhone = true);
    ...
    try {
      ...
      if (field == "phoneNumber") setState(() => _checkingPhone = false);
    }
  }
            ...
            children: [
              Expanded(
                child: TextFormField(
                  ...
                  onChanged: (v) {
                    // Trigger check immediately when input is complete (10 digits)
                    if (v.length == 10) {
                      _checkDuplicate("phoneNumber", v);
                    } else if (_phoneError != null) {
                      // Clear error if they start editing again
  ...
  Widget _input(TextEditingController c, String lbl, IconData ic, 
    ...
    return TextFormField(
      ...
      onChanged: (v) {
        // Immediate completion check for Aadhar
        if (validationType == "aadhar" && v.length == 12) {
