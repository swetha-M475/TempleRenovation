...
class _SignupScreenState extends State<SignupScreen> {
  ...
  // METHOD SELECTION
  String _selectedMethod = 'SMS'; // 'SMS' or 'Phone'
  ...
  Future<void> _sendOTP() async {
    ...
    try {
      // FIX: Check selected method. If 'Phone', use the /VOICE/ endpoint.
      String endpoint = _selectedMethod == 'Phone' ? "VOICE" : "SMS";
      String url = "https://2factor.in/API/V1/$apiKey/$endpoint/+91$phone/AUTOGEN";
      ...
      if (res.statusCode == 200) {
        ...
        if (data["Status"] == "Success") {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text("OTP triggered via $_selectedMethod"))
          );
        }
      }
    } catch (e) {
                        ...
                        child: Form(
                          ...
                          child: Column(
                            children: [
                              ...
                              const SizedBox(height: 16),
                              _phoneSection(), // CUSTOM PHONE SECTION
                              const SizedBox(height: 16),
      ...
      children: [
        if (!_otpVerified) ...[
          ...
          if (_phone.text.length == 10 && !_otpVisible) ...[
            const SizedBox(height: 12),
            Text("Send OTP via:", style: GoogleFonts.poppins(fontSize: 13, color: _mutedBronze)),
            const SizedBox(height: 8),
            Row(
              children: [
                Expanded(child: _methodTile("SMS", Icons.message_outlined)),
                const SizedBox(width: 10),
                Expanded(child: _methodTile("Phone", Icons.phone_callback)),
              ],
            ),
            const SizedBox(height: 12),
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: _otpSending 
                  ...
                  : Text("GET OTP", style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
              ),
            ),
          ]
        ],
  ...
  Widget _otpUI() {
    return Column(
      children: [
        ...
        Text("Sent via $_selectedMethod", style: GoogleFonts.poppins(fontSize: 12, color: _mutedBronze)),
        const SizedBox(height: 10),
        Row(mainAxisAlignment: MainAxisAlignment.center, children: List.generate(6, (i) => _otpBox(i))),
        const SizedBox(height: 15),
        _resendAvailable 
          ? TextButton(onPressed: _sendOTP, child: Text("Resend Code", style: TextStyle(color: _accentColor, fontWeight: FontWeight.bold)))
        ...
        SizedBox(
          ...
          child: ElevatedButton(
            ...
            style: ElevatedButton.styleFrom(backgroundColor: _deepGold),
