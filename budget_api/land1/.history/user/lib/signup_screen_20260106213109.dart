...
class _SignupScreenState extends State<SignupScreen> {
  ...
  final Color _deepGold = const Color(0xFFD4AF37);     // Gold for selection
  ...
  String _selectedMethod = 'SMS'; // Default
  ...
  Future<void> _sendOTP() async {
    ...
    try {
      // Logic: If 'Phone' is selected, you might use a different API endpoint or template.
      // For 2Factor, standard SMS/Call auto-routing is often handled by the service.
      String url = "https://2factor.in/API/V1/$apiKey/SMS/+91$phone/AUTOGEN";
      
      // If your API provider supports a direct 'CALL' flag, update the URL here.
      // Currently using your standard flow as requested.
  ...
  // UPDATED PHONE SECTION WITH SMS/PHONE CHOICE
      ...
      children: [
        if (!_otpVerified) ...[
          ...
          if (_phone.text.length == 10 && !_otpVisible) ...[
            const SizedBox(height: 12),
            Text("Verify via:", style: GoogleFonts.poppins(fontSize: 13, color: _mutedBronze, fontWeight: FontWeight.w500)),
            const SizedBox(height: 8),
            Row(
              children: [
                Expanded(child: _methodTile("SMS", Icons.textsms_outlined)),
                const SizedBox(width: 10),
                Expanded(child: _methodTile("Phone", Icons.phone_in_talk_outlined)),
              ],
            ),
            const SizedBox(height: 12),
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: _otpSending 
                  ...
                  : Text("Get OTP through $_selectedMethod", style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
              ),
            ),
          ]
        ],
  ...
  Widget _otpUI() {
    return Column(
      children: [
        const SizedBox(height: 16),
        Text(
          _selectedMethod == "SMS" ? "Check SMS or WhatsApp for OTP" : "You will receive a phone call",
          style: GoogleFonts.poppins(fontSize: 12, color: _mutedBronze),
        ),
        const SizedBox(height: 12),
        Row(mainAxisAlignment: MainAxisAlignment.center, children: List.generate(6, (i) => _otpBox(i))),
        const SizedBox(height: 10),
        _resendAvailable 
          ? TextButton(onPressed: _sendOTP, child: Text("Resend OTP", style: TextStyle(color: _accentColor, fontWeight: FontWeight.bold)))
          : Text("Resend in $_secondsLeft s", style: TextStyle(color: _mutedBronze)),
        const SizedBox(height: 5),
        ElevatedButton(
          onPressed: _verifyOtp,
          style: ElevatedButton.styleFrom(backgroundColor: _deepGold, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))),
    ...
    return Container(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          ...
          focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: BorderSide(color: _deepGold)),
        ),
        onChanged: (v) {
