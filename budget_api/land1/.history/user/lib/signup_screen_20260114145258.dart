...
class _SignupScreenState extends State<SignupScreen> {
  ...
  // FocusNodes to detect when user leaves a field
  ...
  void initState() {
    super.initState();
    // Listeners to check availability when focus is lost
    _userNode.addListener(() { if (!_userNode.hasFocus) _checkAvailability('username', _username.text); });
    _phoneNode.addListener(() { if (!_phoneNode.hasFocus) _checkAvailability('phoneNumber', _phone.text); });
    _aadharNode.addListener(() { if (!_aadharNode.hasFocus) _checkAvailability('aadharNumber', _aadhar.text); });
  }
  ...
  void dispose() {
    ...
    _nameNode.dispose(); _userNode.dispose(); _phoneNode.dispose();
    _aadharNode.dispose(); _passNode.dispose(); _confirmNode.dispose();
    super.dispose();
  }

  // Check if Username/Phone/Aadhar exists in Firestore
  Future<void> _checkAvailability(String field, String value) async {
    if (value.isEmpty) return;
    try {
      final snap = await _firestore.collection('users').where(field, isEqualTo: value).get();
      setState(() {
        if (snap.docs.isNotEmpty) {
          if (field == 'username') _userError = "Username already taken";
          if (field == 'phoneNumber') _phoneError = "Phone already registered";
          if (field == 'aadharNumber') _aadharError = "Aadhar already registered";
        } else {
          if (field == 'username') _userError = null;
  ...
  Future<void> _signup() async {
    ...
    if (_userError != null || _phoneError != null || _aadharError != null) return;
    if (!_otpVerified) {
  ...
  bool _isValid(String type, String value) {
    ...
    switch (type) {
      ...
      case "username": return value.length >= 4 && _userError == null;
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        child: Form(
                          ...
                          child: Column(
                            children: [
                              _input(_name, "Full Name", Icons.person_outline, node: _nameNode, validationType: "name"),
                              const SizedBox(height: 16),
                              _input(_username, "Username", Icons.alternate_email, node: _userNode, validationType: "username"),
                              const SizedBox(height: 16),
                              _phoneSection(),
                              const SizedBox(height: 16),
                              ...
                              const SizedBox(height: 16),
                              _passwordField(_password, "Password", node: _passNode),
                              const SizedBox(height: 16),
                              _passwordField(_confirm, "Confirm Password", node: _confirmNode, isConfirm: true),
            ...
            children: [
              Expanded(
                child: TextFormField(
                  ...
                  validator: (v) {
                    if (!_phoneNode.hasFocus && (v == null || v.isEmpty)) return "Required";
                    if (_phoneError != null) return _phoneError;
                    if (!_phoneNode.hasFocus && v!.length != 10) return "Enter 10 digits";
                    return null;
                  },
                ),
              ),
                ...
                Padding(
                  ...
                  child: SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      child: _otpSending ? const CircularProgressIndicator(color: Colors.white) : const Text("OTP", style: TextStyle(color: Colors.white)),
                    ),
                  ),
                )
            ],
  ...
  Widget _input(TextEditingController c, String lbl, IconData ic, 
      {TextInputType keyboardType = TextInputType.text, String? validationType, List<TextInputFormatter>? formatters, FocusNode? node}) {
    return TextFormField(
      ...
      onChanged: (v) => setState(() {}),
      validator: (v) {
        if (node != null && node.hasFocus) return null; // Don't show error while typing
        if (v == null || v.isEmpty) return "Required";
        if (validationType == "username" && _userError != null) return _userError;
        if (validationType == "aadhar" && _aadharError != null) return _aadharError;
        if (validationType == "name" && v.length < 3) return "Min 3 characters";
        return null;
      }
    );
  }

  Widget _passwordField(TextEditingController c, String lbl, {bool isConfirm = false, FocusNode? node}) {
    return TextFormField(
      controller: c, 
      focusNode: node,
      obscureText: true, 
      ...
      onChanged: (v) => setState(() {}),
      validator: (v) {
        if (node != null && node.hasFocus) return null;
        if (v == null || v.isEmpty) return "Required";
