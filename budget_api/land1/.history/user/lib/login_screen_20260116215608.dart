...
class _LoginScreenState extends State<LoginScreen> {
  ...
  // Controllers for Username/Phone and Password
  ...
  Future<void> _login() async {
    ...
    try {
      ...
      // 1. Identify if it's a Phone Number (digits only) or Username (letters/numbers)
      bool isPhone = RegExp(r'^[0-9]+$').hasMatch(input);
      debugPrint("Input: $input | IsPhone: $isPhone");

      // 2. Find the user record in Firestore to get the internal sign-in email
      QuerySnapshot query;
      if (isPhone) {
      ...
      // 4. Retrieve the internal email for Firebase Auth
      final userData = query.docs.first.data() as Map<String, dynamic>;
      final String internalEmail = userData['email'] ?? "";

      if (internalEmail.isEmpty) {
        _showError("Account error: This user has no email linked.");
        setState(() => _isLoading = false);
        return;
      }

      // 5. Sign in using the retrieved email and user's password
      debugPrint("Logging in with internal email: $internalEmail");
      ...
      if (userCred.user != null) {
        debugPrint("Success! Redirecting...");
        if (mounted) {
