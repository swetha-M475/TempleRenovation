...
import 'verify_email_screen.dart'; // adjust path if needed
...
class _SignupScreenState extends State<SignupScreen> {
  ...
  // ------------------------
  // Phone listener -> auto-send OTP when 10 digits typed
  // ------------------------
  void _onPhoneChanged() {
    final phone = _phone.text.trim();
    if (!_phoneVerified &&
        phone.length == 10 &&
        _isValidIndianPhone(phone) &&
        !_sendingOtp) {
      _sendOtp2Factor(phone);
    }
  }

  // ------------------------
  // Send OTP via 2Factor service
  // ------------------------
  Future<void> _sendOtp2Factor(String phone) async {
    setState(() {
      _sendingOtp = true;
      _otpVisible = false;
    ...
    try {
      ...
      if (result['status'] == 'Success' && result['sessionId'] != null) {
        setState(() {
          _otpVisible = true;
      ...
      _showSnackbar('OTP send failed: $e');
    } finally {
  ...
  void _resendOtp() {
    ...
    final phone = _phone.text.trim();
    if (_isValidIndianPhone(phone)) _sendOtp2Factor(phone);
  }

  // ------------------------
  // Verify OTP via 2Factor service
  // ------------------------
  Future<void> _verifyOtp2Factor() async {
    ...
    if (_sessionId.isEmpty) {
      _showSnackbar('OTP session not available. Send OTP again.');
      return;
    }

    setState(() => _sendingOtp = true);
    try {
      final ok = await OTPService.verifyOTP(_sessionId, otp);
      if (ok) {
        setState(() {
          _phoneVerified = true;
          _otpVisible = false;
  ...
  // ------------------------
  // Create account using Firebase Email+Password
  // (phone already verified via 2Factor)
  // ------------------------
  Future<void> _createAccount() async {
    ...
    try {
      ...
      // phone check in users collection
      ...
      await _firestore.collection('users').doc(user.uid).set(doc);
  ...
  Widget _phoneField() {
    return Column(
      ...
      children: [
        ...
        // Show OTP row only if session exists and not verified
        if (_sessionId.isNotEmpty && !_phoneVerified) _buildOtpArea(),
      ],
    );
  }
      ...
      children: [
        ...
        Row(
          ...
          children: [
            ...
            ElevatedButton(
              onPressed: _sendingOtp ? null : _verifyOtp2Factor,
              style: ElevatedButton.styleFrom(
