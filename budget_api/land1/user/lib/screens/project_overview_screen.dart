...
class _ProjectOverviewScreenState extends State<ProjectOverviewScreen>
  ...
  Future<void> _showAddAdditionalRequirementDialog(List<dynamic> currentWorks) async {
    final nameCtrl = TextEditingController();
    final descCtrl = TextEditingController();
    ...
    await showDialog(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setDialogState) {
          return AlertDialog(
            backgroundColor: backgroundCream,
            title: Text('Request Additional Requirement', style: GoogleFonts.poppins(color: primaryMaroon, fontWeight: FontWeight.bold, fontSize: 16)),
            content: SingleChildScrollView(
              child: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  ...
                  TextField(
                    ...
                    decoration: const InputDecoration(labelText: 'Estimated Amount (Optional)', prefixText: '₹ '),
                  ),
                  ...
                  ElevatedButton.icon(
                    ...
                    label: Text(selectedImage == null ? 'Upload Reference Image' : 'Image Selected'),
                  ),
                  if (isSubmitting)
                    const Padding(padding: EdgeInsets.only(top: 20), child: CircularProgressIndicator(color: primaryMaroon)),
                ],
              ),
            ),
            actions: [
                ...
                onPressed: isSubmitting ? null : () async {
                  ...
                  try {
                    ...
                    if (selectedImage != null) {
                      imageUrl = await CloudinaryService.uploadImage(imageFile: selectedImage!, userId: _userId, projectId: _projectId);
                    }

                    Map<String, dynamic> newWork = {
                      'workName': nameCtrl.text.trim(),
                      'workDescription': descCtrl.text.trim(),
                      'amount': amountCtrl.text.trim(),
                      'workImageUrl': imageUrl ?? '',
                      'isAdditional': true,
                      'requestDate': Timestamp.now(), 
                      'distributionMethod': '', // Admin to fill later
                    };

                    List<dynamic> updatedWorks = List.from(currentWorks)..add(newWork);

                    // We DO NOT automatically increase the 'estimatedAmount' here.
                    // The admin will review this new work item and assign/approve the budget later.
                    await _firestore.collection('projects').doc(_projectId).update({'works': updatedWorks});

                    if (mounted) {
                      Navigator.pop(context);
                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Requirement requested successfully!'), backgroundColor: successGreen));
                    }
                  } catch (e) {
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setDialogState) => AlertDialog(
            ...
            child: Column(
              ...
              children: [
                ...
                ElevatedButton.icon(
                  ...
                  icon: const Icon(Icons.image),
                  label: Text(selectedFiles.isEmpty ? 'Select Bill Photos (Optional)' : '${selectedFiles.length} Images Selected'),
                ),
                if (isUploading)
                  const Padding(padding: EdgeInsets.only(top: 20), child: CircularProgressIndicator(color: primaryMaroon)),
              ],
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: isUploading ? null : () async {
                // Modified Validation: selectedFiles is no longer required
                ...
                try {
                  ...
                  await _billsRef.add({
                    ...
                    'imageUrls': urls, // Will just be an empty array if no images were selected
                    'createdAt': FieldValue.serverTimestamp(),
                  });
                  if (mounted) {
                    Navigator.pop(context);
  ...
  Future<int> _getCompletedTasksCount() async {
    final snap = await _firestore.collection('project_tasks').where('projectId', isEqualTo: _projectId).where('status', isEqualTo: 'completed').limit(1).get();
    return snap.docs.length;
  }

  Future<int> _getOngoingTasksCount() async {
    final snap = await _firestore.collection('project_tasks').where('projectId', isEqualTo: _projectId).where('status', whereIn: ['ongoing', 'pending_approval']).limit(1).get();
    return snap.docs.length;
  }

  Future<int> _getTodoTasksCount() async {
    final snap = await _firestore.collection('project_tasks').where('projectId', isEqualTo: _projectId).where('status', isEqualTo: 'todo').limit(1).get();
    return snap.docs.length;
  }
  ...
  Future<void> _requestProjectCompletion() async {
    ...
    try {
      ...
      if (completed == 0) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Complete at least one task before requesting completion.')));
        return;
      }
      if (ongoing > 0 || todo > 0) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('All tasks must be finished or deleted first.')));
        return;
      }
      ...
      await _firestore.collection('project_messages').add({
        'projectId': _projectId,
        'senderRole': 'user',
        'message': 'User has requested to mark this project as completed.',
        'type': 'system_completion_request',
        'createdAt': FieldValue.serverTimestamp(),
      });

      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Completion request sent! check the Feedback tab.')));
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error: $e')));
    } finally {
  ...
  Widget _userFinanceTab() {
    return StreamBuilder<DocumentSnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        List<dynamic> works = projectData['works'] ?? [];
        String totalBudget = projectData['estimatedAmount']?.toString() ?? '0';

        return Column(
          children: [
            // Floating Top Summary
            Container(
              margin: const EdgeInsets.all(16),
              padding: const EdgeInsets.all(16),
              decoration: BoxDecoration(
                gradient: const LinearGradient(colors: [primaryMaroon, Color(0xFF4A1010)]),
                borderRadius: BorderRadius.circular(12),
              ),
              child: Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  const Text("Total Approved Budget", style: TextStyle(color: Colors.white70, fontSize: 14)),
                  Text("₹ $totalBudget", style: const TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold)),
                ],
              ),
            ),
            ...
            Expanded(
              child: ListView.builder(
                ...
                itemCount: works.length + 1, // +1 for the button at the end
                itemBuilder: (context, index) {
                  if (index == works.length) {
                    return Padding(
                      padding: const EdgeInsets.only(top: 16, bottom: 40),
                      child: OutlinedButton.icon(
                        onPressed: () => _showAddAdditionalRequirementDialog(works),
                        icon: const Icon(Icons.add_circle_outline),
                        label: const Text("Request Additional Requirement"),
                  ...
                  Map<String, dynamic> work = Map<String, dynamic>.from(works[index]);
                  bool isAdditional = work['isAdditional'] == true;
                  String method = work['distributionMethod'] ?? '';
                  bool hasAmount = work['amount'] != null && work['amount'].toString().isNotEmpty;
                      ...
                      child: Column(
                        ...
                        children: [
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    Row(
                                      ...
                                      children: [
                                        Expanded(
                                          child: Text(work['workName'] ?? "Work Item", style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                                        ),
                                        if (isAdditional)
                                          Container(
                                    ...
                                    Text(work['workDescription'] ?? "", style: const TextStyle(fontSize: 12, color: Colors.grey)),
                                  ],
                                ),
                              ),
                            ],
                          ...
                          Row(
                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                            children: [
                              const Text("Approved Amount:", style: TextStyle(fontWeight: FontWeight.w500, fontSize: 13)),
                              Text(
                                hasAmount ? "₹ ${work['amount']}" : "Pending Admin Update",
                                style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: hasAmount ? successGreen : Colors.orange),
                              )
                            ],
                          ),
                                ...
                                children: [
                                  const Row(
                                    children: [
                                      ...
                                      SizedBox(width: 6),
                                      Text("Amount Transferred by Admin", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: Colors.blue)),
                                    ],
                                  ),
                                  const SizedBox(height: 6),
                                  Text("Txn ID: ${work['transactionId'] ?? 'N/A'}", style: const TextStyle(fontSize: 12)),
                                  Text("Date: ${_formatTimestamp(work['transactionDate'])}", style: const TextStyle(fontSize: 12)),
                                ],
                              ),
                            )
                                ...
                                children: [
                                  const Row(
                                    children: [
                                      ...
                                      SizedBox(width: 6),
                                      Text("Materials Sent by Admin", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: Colors.orange)),
                                    ],
                                  ),
                                  const SizedBox(height: 6),
                                  Text("Req Date: ${_formatTimestamp(work['requestDate'])}", style: const TextStyle(fontSize: 12)),
                                  Text("Sent Date: ${_formatTimestamp(work['dateSent'])}", style: const TextStyle(fontSize: 12)),
                                ],
                              ),
                            )
                          ] else ...[
                            Container(
                              ...
                              child: const Row(
                                children: [
                                  ...
                                  SizedBox(width: 6),
                                  Text("Awaiting Admin Processing...", style: TextStyle(fontSize: 12, color: Colors.grey, fontStyle: FontStyle.italic)),
                                ],
                              ),
                            )
                          ]
  ...
  Widget _billsTabWrapper() {
    return Column(
      children: [
        const SizedBox(height: 16),
        // Action Buttons inside Bills Tab
        Padding(
            ...
            children: [
              SizedBox(
                ...
                child: OutlinedButton.icon(
                  ...
                  label: Text(_isCompletionRequesting ? 'Sending completion request...' : 'Request Project Completion'),
                  style: OutlinedButton.styleFrom(
                      ...
                      children: [
                        if (imageUrls.isNotEmpty)
                          ...
                          const Padding(
                            ...
                            child: Text('No images attached to this bill.', style: TextStyle(color: Colors.grey, fontStyle: FontStyle.italic)),
                          )
                      ],
                    ),
                  );
          ...
          Padding(
            ...
            child: SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                style: ElevatedButton.styleFrom(backgroundColor: primaryMaroon, foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
              ),
            ),
          ),
          Container(
  ...
  Widget _buildTodoList() {
    return StreamBuilder<QuerySnapshot>(
      stream: _firestore.collection('project_tasks').where('projectId', isEqualTo: _projectId).where('status', isEqualTo: 'todo').orderBy('createdAt', descending: false).snapshots(),
      builder: (context, snapshot) {
        if (!snapshot.hasData) return const Center(child: CircularProgressIndicator());
        final docs = snapshot.data!.docs;
  ...
  Widget _buildOngoingList() {
    return StreamBuilder<QuerySnapshot>(
      stream: _firestore.collection('project_tasks').where('projectId', isEqualTo: _projectId).where('status', whereIn: ['ongoing', 'pending_approval']).snapshots(),
      builder: (context, snapshot) {
        if (!snapshot.hasData) return const Center(child: CircularProgressIndicator());
        final docs = snapshot.data!.docs;
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            final String dateDisplay = startTs != null ? "Started: ${_formatTimestamp(startTs)}" : "Date not captured";
  ...
  Widget _buildCompletedList() {
    return StreamBuilder<QuerySnapshot>(
      stream: _firestore.collection('project_tasks').where('projectId', isEqualTo: _projectId).where('status', isEqualTo: 'completed').orderBy('completedAt', descending: true).snapshots(),
      builder: (context, snapshot) {
        if (!snapshot.hasData) return const Center(child: CircularProgressIndicator());
        final docs = snapshot.data!.docs;
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            final data = docs[index].data() as Map<String, dynamic>;
            final String startStr = _formatTimestamp(data['startedAt']);
            final String endStr = _formatTimestamp(data['completedAt']);
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        Expanded(child: Text(data['taskName'] ?? '', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16))),
...
class _OngoingTaskCardState extends State<OngoingTaskCard> {
  ...
  Future<void> _deleteTask() async {
    ...
    if (confirm) await FirebaseFirestore.instance.collection('project_tasks').doc(widget.taskId).delete();
  }

  Future<void> _sendForApproval() async {
    if (_selectedImages.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Please upload an image before sending for approval"), backgroundColor: Colors.red));
      return;
    }
    ...
    try {
      ...
      for (var image in _selectedImages) {
        final url = await CloudinaryService.uploadImage(imageFile: image, userId: widget.userId, projectId: widget.projectId);
  ...
  @override
  Widget build(BuildContext context) {
    final bool isPending = widget.currentStatus == 'pending_approval';
            ...
            if (!isPending) ...[
              ...
              Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      _selectedImages.isEmpty ? "No file chosen" : "${_selectedImages.length} files selected",
